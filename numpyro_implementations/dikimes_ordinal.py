...
def sample_prior(data, observed_data):
    ...
    cutpoints = dist.TransformedDistribution(dist.Normal(0,10).expand([N_classes - 1]),
                                             dist.transforms.OrderedTransform(),).sample(random.PRNGKey(0),
                                                                                         (num_samples,))
...
for i in range(5):
    ...
    # marginal_prior = log_likelihood(logistic_regression_model, prior_samples, exp_data_X_Z2, exp_observed_data)
    #
    # print('X,Z2 prior', jnp.sum(logsumexp(marginal_prior['obs'], 0) - jnp.log(num_samples)))
    #posterior sampling
    ...
    # marginal_prior = log_likelihood(logistic_regression_model, prior_samples, exp_data_X, exp_observed_data)
    #
    # print('X prior', jnp.sum(logsumexp(marginal_prior['obs'], 0) - jnp.log(num_samples)))
    #posterior sampling
    ...
    # marginal_posterior = log_likelihood(logistic_regression_model, posterior_samples, exp_data_X, exp_observed_data)
    #
    # print('X posterior', jnp.sum(logsumexp(marginal_posterior['obs'], 0) - jnp.log(num_samples)))
